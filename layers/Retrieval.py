...
class RetrievalTool(nn.Module):
    ...
    def context_similarity(self, meta_query, bsz, device, require_grad=False):
        ...
        if require_grad and self.train_context_encoder:
            query_ctx = self.context_encoder(query_meta)  # [G, B, D]
            pool_ctx = self.meta_pool_context.to(device).detach()  # [G, T, D]
            sim_ctx = torch.einsum(
                "gbd,gtd->gbt",
                F.normalize(query_ctx, dim=-1),
                F.normalize(pool_ctx, dim=-1),
            )
        else:
            with torch.no_grad():
                query_ctx = self.context_encoder(query_meta)  # [G, B, D]
    ...
    def retrieve(self, x, index, meta_query=None, train=True):
        ...
        _, coarse_idx = torch.topk(sim_wave, coarse_k, dim=2)

        # Stage-2 contextual reranking: choose top-m by context similarity only
        # inside the waveform-retrieved coarse candidate pool.
        require_grad = train and self.train_context_encoder and torch.is_grad_enabled()
        sim_context = self.context_similarity(meta_query, bsz, x.device, require_grad=require_grad)  # [G, B, T]
        coarse_context = torch.gather(sim_context, dim=2, index=coarse_idx)

        select_k = min(self.topm, coarse_k)
        selected_score, selected_local_idx = torch.topk(coarse_context, select_k, dim=2)
        selected_global_idx = torch.gather(coarse_idx, dim=2, index=selected_local_idx)

        ranking_prob = F.softmax(selected_score / self.temperature, dim=2)
